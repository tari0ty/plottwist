      ...
      <section className='mx-auto flex min-h-screen w-full max-w-6xl items-center justify-center px-4 py-16 sm:px-6 lg:px-8'>
        <div className='grid w-full gap-8 rounded-3xl border border-white/10 bg-white/6 p-6 shadow-2xl shadow-black/30 backdrop-blur md:grid-cols-[1.05fr_0.95fr] md:p-10'>
          <article className='space-y-6 rounded-3xl border border-white/8 bg-black/30 p-6 md:p-8'>
            ...
            <h1 className='max-w-md text-4xl font-black tracking-tight text-white md:text-5xl'>Start with a secure account.</h1>
            <p className='max-w-md text-base text-slate-300 md:text-lg'>Create your account with Google or email, then continue into your profile setup.</p>
          </article>
          <article className='rounded-3xl border border-white/10 bg-[#0b1622] p-6 md:p-8'>
            <div className='mb-6 flex items-center justify-between gap-3'>
              <div>
                <p className='text-sm uppercase tracking-[0.25em] text-[#e8d5b7]'>Sign up</p>
                <h2 className='text-2xl font-bold text-white'>Create your account</h2>
              </div>
              <Link href='/login' className='text-sm font-semibold text-[#e8d5b7] hover:text-[#f0e3ce]'>Log in</Link>
            </div>
            ...
            <form onSubmit={handleEmail} className='space-y-4'>
              ...
              <label className='block text-sm text-slate-200'>
                Password
                <input type='password' value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className='mt-1 w-full rounded-2xl border border-slate-700 bg-slate-950/90 px-4 py-3 text-white outline-none transition placeholder:text-slate-400 focus:border-cyan-400' placeholder='At least 6 characters' />
              </label>
